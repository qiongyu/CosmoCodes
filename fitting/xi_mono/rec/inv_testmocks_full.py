...
a = np.loadtxt('red_rec.txt',dtype=({'names': ['infile'],'formats': ['S100']}))
infile = a['infile']
nf = np.size(infile)
if(stemp == 'f'):
  snl = signl_ap
...
for i in range(0,nf):
  ...
  rin, data = np.loadtxt(infile[i], unpack=True, usecols=(0,1))
...
#store model in this
  ...
  pyplot.figtext(0.57,0.63,r'$b_0^2 = $'+str(b0bf[i]))
  for pp in range(0,npt):
    pyplot.figtext(0.57,0.63-0.02*(pp+1),r'$a_{%d} = {%f}$' % (pp+1, ap[pp,i]))
  pyplot.figtext(0.57,0.63-0.02*(npt+1),r'$\alpha = $'+str(opt[0]))
  pyplot.figtext(0.57,0.63-0.02*(npt+2),r'$\chi^2 = $'+str(chibf[i]))
  ...
  for pp in range(0,npt):
    pyplot.figtext(0.15,0.18-0.02*(pp+1),r'$a_{%d} = {%f}$' % (pp+1, ap[pp,i]))
