...
def issequence(x):
    "Is x a sequence? We say it is if it has a __iter__ method."
...
class Spline:
	...
	self.x_vals = numpy.array(x_array)
	self.y_vals = numpy.array(y_array)
	...
	"Simulate a ufunc; handle being called on an array."
        if issequence(arg):
            return numpy.array(map(self.call, arg))
	else:
